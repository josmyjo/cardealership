...
$result=mysqli_query ($con,"SELECT *,s.email as emails,s.name as names, s.dob as dobs, s.contactNumber as contactNumbers FROM service s JOIN variant v ON s.varient_id = v.variant_id JOIN  branch b ON s.branchID=b.branchID JOIN carmodel c ON s.modelName = c.modelID WHERE s.branchID=$manager");
